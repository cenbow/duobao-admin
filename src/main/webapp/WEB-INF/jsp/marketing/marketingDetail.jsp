						...
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
							<div class="panel panel-default bk-bg-white">
								<div class="panel-heading bk-bg-white">
									<h6><i class="fa fa-hand-o-right"></i><span class="break"></span>营销活动详情</h6>							
								...
								<div class="panel-body bk-bg-white bk-padding-top-30 bk-padding-bottom-20">
									<form action="updateMarket.html"  class="form-horizontal form-bordered " id="editForm" method="post">
									<input type="hidden" name="marketId" value="<c:out value="${marketing.id}"/>">
                                        <div class="form-group">
                                            <label class="col-sm-3 control-label" for="remark">活动名称</label>
                                            <div class="col-md-3">
                                               <input type="text" id="name" name="name" class="form-control" value='<c:out value="${marketing.name}"/>'/>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-sm-3 control-label" for="remark">活动开始时间</label>
                                            <div class="col-md-3">
                                               <input type="text" class="form-control" name="startTime" id="startTime"  placeholder="开始时间"  value='<fmt:formatDate value="${marketing.gmt_validity_start}" pattern="yyyy-MM-dd  HH:mm:ss" />'>
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label class="col-sm-3 control-label" for="remark">活动结束时间</label>
                                            <div class="col-md-3">
                                              <input type="text" class="form-control" name="endTime" id="endTime"  placeholder="结束时间"  value='<fmt:formatDate value="${marketing.gmt_validity_end}" pattern="yyyy-MM-dd  HH:mm:ss" />' >
                                            </div>
                                        </div>
                                       <div class="form-group">
                                         ...
                                         <c:forEach var="dis" items="${discountlist}">
                                          <label class="col-sm-3 control-label" for="remark"></label>
                                          <div class="row" >
													<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
														<input type="number" name="limit_amount" class="form-control" value='<fmt:formatNumber    value="${dis.limit_amount/100}"   type="number" pattern="0" />' placeholder="100" style="width:100px;" readonly/>
													</div>
													<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
														 <input type="number" id="numbers" name="reduce_amount" class="form-control" value='<fmt:formatNumber    value="${dis.reduce_amount/100}"   type="number" pattern="0" />' placeholder="99" style="width:100px;" readonly/>
													</div>
													<div class="col-lg-2 col-md-2 col-sm-2 col-xs-2">
														<input type="text"  name="discount" class="form-control" value='<c:out value="${dis.discount}"/>' placeholder="0.8" style="width:100px;" readonly />
													</div>
													<a href='disCountDetail.html?id=<c:out value="${dis.id}"/>' class="btn btn-success"><i class="fa fa-search-plus">添加红包触发计划</i></a>
											</div>
										</c:forEach>
                                         </div>
                                        <div class="row">
